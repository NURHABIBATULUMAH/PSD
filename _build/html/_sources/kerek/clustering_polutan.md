...
cols_to_drop = [c for c in ['id', 'nama', 'daerah'] if c in df.columns]
fitur = df.drop(cols_to_drop, axis=1) if cols_to_drop else df
...
# 1. Memuat Data
df = pd.read_csv('NO2-Kerek-TSFEL.csv')
identitas_cols = [c for c in ['id', 'nama', 'daerah'] if c in df.columns]
identitas = df[identitas_cols] if identitas_cols else pd.DataFrame({'daerah': ['Kerek'] * len(df)})
cols_to_drop = [c for c in ['id', 'nama', 'daerah'] if c in df.columns]
fitur = df.drop(cols_to_drop, axis=1) if cols_to_drop else df
...
## 3. Interpretasi Hasil Clustering (Scatter Plot)

```{figure} ./img/clustering_polutan/scatter_plot.png
---
name: scatterplot-kmeans
align: center
width: 70%
---
Visualisasi Scatter Plot Hasil Clustering K-Means
```

Berdasarkan visualisasi grafik *Scatter Plot* yang dihasilkan, algoritma K-Means telah membagi data menjadi **6 kelompok (cluster_0 hingga cluster_5)**. Sumbu X menunjukkan nama daerah (Manyar, Gresik Kota, Cerme, dll), sedangkan Sumbu Y menunjukkan penetapan Cluster.
